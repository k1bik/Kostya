...
public class ResultGroup {
    ...
    public ResultGroup filterBaseName(String filter){ 
        ResultGroup group = new ResultGroup (name+": результаты для команд на \"" + filter +"\"");
        for (Result res : results)
            if (res.getBaseName().startsWith(filter))group.addResult(res);
        return group;
    }
    ...
    public int baseNumber(){
        ...
        for (Result res:results)
        	trainNsumber.add(res.getBaseName()); 
    ...
    public List <TotalRecord> totalMaxPriceOfTicket(){
        ...
        for (Result res:results)
            commandsS.add(res.getVacationerStatus());
        ...
        for (int i=0; i<m; i++){
            ...
            com = commandsL.get(i);
            for (Result res: results){
                if (com.equals(res.getVacationerStatus()))
